...
public class GameLogic {
    ...
    private String[] playerNames = {"Gracz 1", "Gracz 2"};
    ...
    public boolean updateGameBoard(int row, int col){
        if ( gameBoard [row -1][col-1] == 0){
            ...
            if(player == 1)
            {
                playerTurn.setText(playerNames[1] + " twoja kolej!");
            }
            else
    ...
    public boolean winnerCheck(){
        ...
        if(isWinner){
            ...
            playerTurn.setText((playerNames[player-1] + " *wygrał  EZ*"));
            return true;
        }
        else if(boardFilled == 9){
            playAgainBTN.setVisibility(View.VISIBLE);
            homeBTN.setVisibility(View.VISIBLE);
            playerTurn.setText("GG to jest remis!");
            return true;
        }
        else
            return false;
